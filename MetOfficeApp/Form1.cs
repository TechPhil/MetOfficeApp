...
namespace MetOfficeApp
{
    public partial class Form1 : Form
    {
        ...
        {
            //Setup Weather Codes
            var weather_reader = new StreamReader(File.OpenRead("Weather_Codes.txt"));
            ...
            var visibility_reader = new StreamReader(File.OpenRead("Visibility.txt"));
            ...
            var uv_reader = new StreamReader(File.OpenRead("UV.txt"));
        ...
        private void GetWeather(int choice)
        {
            ...
            try
            {
                var locationone = client.GetClosestSite(coords); //Find closest weather site to postcode
            ...
            dynamic forecastResponse = null; //Defines forecastResponse variable as dynamic, so it can be used regardless of which type the user requests
            if (choice.Equals(1)) //Is choice Three Hourly?
            {
                try
                {
            ...
            else if (choice.Equals(2)) //Is choice Daily?
            {
                try
                {
